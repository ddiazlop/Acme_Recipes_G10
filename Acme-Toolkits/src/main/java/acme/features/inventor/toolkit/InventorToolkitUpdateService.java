...
public class InventorToolkitUpdateService implements AbstractUpdateService<Inventor, Toolkit> {
	...
	public void validate(final Request<Toolkit> request, final Toolkit entity, final Errors errors) {
		...
		if (!errors.hasErrors("title")) {
			final Boolean hasNoSpamTitle = spamDetector.stringHasNoSpam(entity.getTitle());
			errors.state(request, hasNoSpamTitle, "title", "spam.detector.error.message");
		}
		if (!errors.hasErrors("description")) {
			final Boolean hasNoSpamDescription = spamDetector.stringHasNoSpam(entity.getDescription());
			errors.state(request, hasNoSpamDescription, "description", "spam.detector.error.message");
		}
		if (!errors.hasErrors("assemblyNotes")) {
			final Boolean hasNoSpamAssemblyNotes = spamDetector.stringHasNoSpam(entity.getAssemblyNotes());
			errors.state(request, hasNoSpamAssemblyNotes, "assemblyNotes", "spam.detector.error.message");
		}
	}

...
public class AnyChirpCreateService implements AbstractCreateService<Any, Chirp> {
	...
	public void validate(final Request<Chirp> request, final Chirp entity, final Errors errors) {
		...
		if(!errors.hasErrors("author")) {
			final Boolean hasNoSpamAuthor = spamDetector.stringHasNoSpam(entity.getAuthor());
			errors.state(request, hasNoSpamAuthor, "author", "spam.detector.error.message");
		}
		
		if (!errors.hasErrors("body")) {
			final Boolean hasNoSpamBody = spamDetector.stringHasNoSpam(entity.getBody());
			errors.state(request, hasNoSpamBody, "body", "spam.detector.error.message");
		}
